...
import schema
from auditmiami import update_name, mapping

OSM_PATH = "miamimap.osm"
    ...
    if element.tag == 'node':
        for attrib in element.attrib:
            if attrib in NODE_FIELDS:
                if (attrib == "lat") or (attrib == "lon"):
                    node_attribs[attrib] = float(element.get(attrib))
                else:
        ...
        for child in element:
            ...
            if child.tag == 'tag':
                ...
                nd_way['id'] = element.attrib['id']
                nd_way['node_id'] = child.attrib['ref']
                nd_way['position'] = position 
                position += 1                
                way_nodes.append(nd_way)
